...
export default function Game(){
  ...
  return <main className={`game-shell ${playing?'is-playing':''}`}>
    ...
    {!ready&&<>
      ...
      {playing&&<><div className="crosshair"/><div className="goal-pill"><Flag size={16}/><span>Reach the flag</span><Progress className="goal-track" value={Math.round(state.progress*100)} aria-label="Progress to flag"/></div><div className="playing-hint"><kbd>SPACE</kbd> Jump <span>·</span> <kbd>ESC</kbd> Pause <span>·</span> Jump on Goombas to stomp</div><div className="touch-controls"><div className="touch-pad" onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);const r=e.currentTarget.getBoundingClientRect();engine.current?.setTouch((e.clientX-r.left-r.width/2)/(r.width/2),(e.clientY-r.top-r.height/2)/(r.height/2));}} onPointerMove={e=>{if(e.buttons){const r=e.currentTarget.getBoundingClientRect();engine.current?.setTouch((e.clientX-r.left-r.width/2)/(r.width/2),(e.clientY-r.top-r.height/2)/(r.height/2));}}} onPointerUp={()=>engine.current?.setTouch(0,0)} onPointerCancel={()=>engine.current?.setTouch(0,0)} aria-label="Movement joystick"><span>✣</span></div><button onPointerDown={e=>{e.preventDefault();engine.current?.jump();}} className="touch-jump" aria-label="Jump"><MoveUp size={24}/>JUMP</button></div></>}
      {state.notice&&<div className="game-notice" role="status">{state.notice}</div>}
      {!playing&&<div className="state-overlay"><section className="state-card"><div className="state-icon">{state.status==='won'?<Trophy size={34}/>:state.status==='paused'?<Pause size={32}/>:<Heart size={32}/>}</div><div className="eyebrow">WORLD 1–1</div><h2>{state.status==='won'?'Course clear!':state.status==='paused'?'Take a breather.':'One more try?'}</h2><p>{state.status==='won'?`You made it! ${state.coins} coins collected · ${state.score.toLocaleString()} points.`:state.status==='paused'?'The Mushroom Kingdom will be right here.':'Every adventure takes a little practice.'}</p><button className="play-button" onClick={()=>state.status==='paused'?start():engine.current?.restart()}><span>{state.status==='paused'?'Keep playing':'Play again'}</span>{state.status==='paused'?<Play size={20}/>:<RotateCcw size={20}/>}</button>{state.status==='paused'&&<button className="text-button" onClick={()=>engine.current?.restart()}>Restart course</button>}</section></div>}
    </>}
  </main>;
}
