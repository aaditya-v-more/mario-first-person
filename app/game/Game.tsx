  ...
  return <main ref={shell} className={`game-shell ${playing?'is-playing':''}`}>
    ...
    {ready&&<>
      ...
      <section className="intro">
        ...
        <AppearanceControls/><CreatorLinks/>
    ...
    {!ready&&<>
      ...
      {!playing&&<div className="state-overlay"><section className="state-card"><div className="state-icon">{state.status==='won'?<Trophy size={34}/>:state.status==='paused'?<Pause size={32}/>:<Heart size={32}/>}</div><div className="eyebrow">WORLD 1–1</div><h2>{state.status==='won'?'Course clear!':state.status==='paused'?'Take a breather.':'One more try?'}</h2><p>{state.status==='won'?`You made it! ${state.coins} coins collected · ${state.score.toLocaleString()} points.`:state.status==='paused'?'The Mushroom Kingdom will be right here.':'Every adventure takes a little practice.'}</p><button className="play-button" onClick={()=>start(state.status!=='paused')}><span>{state.status==='paused'?'Keep playing':'Play again'}</span>{state.status==='paused'?<Play size={20}/>:<RotateCcw size={20}/>}</button>{state.status==='paused'&&<button className="text-button" onClick={()=>start(true)}>Restart course</button>}<AppearanceControls/><CreatorLinks/>{displayState.message&&<output className="display-note">{displayState.message}</output>}</section></div>}
    </>}
  </main>;
}
