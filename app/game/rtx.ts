...
export class RtxGraphics {
  ...
  enable(width: number, height: number) {
    ...
    const original = {
      ...
      exposure: renderer.toneMappingExposure, pixelRatio: renderer.getPixelRatio(),
      sunColor: sun.color.clone(), sunIntensity: sun.intensity,
      shadowRadius: sun.shadow.radius, shadowBias: sun.shadow.normalBias,
      skyColor: hemisphere.color.clone(), groundColor: hemisphere.groundColor.clone(),
    ...
    renderer.debug.onShaderError = (...args) => { shaderFailed = true; previousShaderError?.(...args); };
    try {
      this.sky = this.makeSky();
      scene.background = this.sky;
      scene.fog = new THREE.Fog('#c8e4ed', 65, 205);
      sun.color.set('#ffe2ab');
      sun.intensity = 4.2;
      sun.shadow.radius = 1.75;
      sun.shadow.normalBias = .045;
      hemisphere.color.set('#b9dcff');
      hemisphere.groundColor.set('#8caa54');
      hemisphere.intensity = .95;
      ...
      try { this.environment = pmrem.fromScene(scene, .02, .1, 260, { size: 128, position }); }
      ...
      renderer.setPixelRatio(graphicsPixelRatio(width, height, window.devicePixelRatio || 1));
      this.composer = new EffectComposer(renderer);
      this.composer.addPass(new RenderPass(scene, this.camera));
      this.ao = new GTAOPass(scene, this.camera, 1, 1);
      this.ao.updateGtaoMaterial({ radius: .6, thickness: .5, distanceFallOff: 1, samples: 16 });
      this.ao.updatePdMaterial({ radius: 8, samples: 16, depthPhi: 4, normalPhi: 4 });
      this.ao.blendIntensity = .75;
      this.composer.addPass(this.ao);
      this.composer.addPass(new UnrealBloomPass(new THREE.Vector2(1, 1), .32, .45, 1.15));
  ...
  private makeSky() {
    ...
    const zenith = new THREE.Color('#168ad1'), horizon = new THREE.Color('#a1dff4');
    const ground = new THREE.Color('#738552'), color = new THREE.Color();
    ...
    for (let y = 0; y < height; y++) {
      ...
      for (let x = 0; x < width; x++) {
        ...
        direction.set(Math.cos(latitude) * Math.cos(longitude), up, Math.cos(latitude) * Math.sin(longitude));
        color.copy(horizon).lerp(up > 0 ? zenith : ground, Math.pow(Math.abs(up), .45));
        const alignment = Math.max(0, direction.dot(sunDirection));
        const glow = Math.pow(alignment, 64) * .5 + Math.pow(alignment, 2400) * 18;
        const i = (y * width + x) * 4;
        data[i] = THREE.DataUtils.toHalfFloat(color.r + glow);
        data[i + 1] = THREE.DataUtils.toHalfFloat(color.g + glow * .83);
        data[i + 2] = THREE.DataUtils.toHalfFloat(color.b + glow * .58);
  ...
  resize(width: number, height: number) {
    ...
    const ratio = graphicsPixelRatio(width, height, window.devicePixelRatio || 1);
    this.renderer.setPixelRatio(ratio);
    this.composer.setPixelRatio(ratio);
    this.composer.setSize(width, height);
    // AO at half resolution keeps the effect affordable while moving.
    this.ao.setSize(Math.max(1, Math.round(width * ratio / 2)), Math.max(1, Math.round(height * ratio / 2)));
    this.fxaa.uniforms.resolution.value.set(1 / Math.max(1, width * ratio), 1 / Math.max(1, height * ratio));
  }
  ...
  dispose() {
    ...
    this.sky?.dispose();
    this.composer = this.ao = this.fxaa = this.environment = this.sky = undefined;
